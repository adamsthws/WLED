...
class UsermodBattery : public Usermod
{
  private:
    ...
    unsigned long initialDelay = 15000; // Initial delay of in milliseconds
    bool initialDelayComplete = false;  // Flag to check if the initial delay is completed
    // battery min. voltage
  ...
  public:
    ...
    nextReadTime = millis() + initialDelay; // Adjust the first reading time by the initial delay
    lastReadTime = millis();
    initialDelayComplete = false; // Mark that the initial delay is not yet completed
    initDone = true;
  }
  ...
  void loop()
  {
    ...
    if (!initialDelayComplete)
    {
      ...
      nextReadTime = millis() + readingInterval; // Set the regular interval after initial delay
    }

    if (millis() < nextReadTime)
      return; // Regular interval handling
...
*/
    ...
    // LiPo batteries have a different discharging curve, see
...
#endif
    if (voltage > -1.0f)
      batteryLevel = constrain(batteryLevel, 0.0f, 110.0f);
